<script>
    ...
    async function readFileData(dataName, repeatName){
        ...
        try {
            ...
            dataToRender = await getZarrLoci(repeatName, dataFile);
            dataToRender.sort((a, b) => parseInt(a.key.replace('chr', '')) - parseInt(b.key.replace('chr', '')))
        } catch (error) {
            alert("This repeat is not contained in the dataset.");
            status = "hidden";
            return 0
            // Code to handle the error
            console.error(`An error occurred: ${error.message}`);
        } finally {
            // Optional 'finally' block that is executed regardless of whether an error occurred
        }

        let valuesList = $Cart.multiTEGenomeList;
        const mappedDataToRender = dataToRender.map(list => {
            let mappedList = list;
            mappedList['values'] = list['values'].map(v => {
                v['data'] = dataName;
                return v
            })
            return mappedList
        })
        valuesList[repeatName] = mappedDataToRender;
        // let dataSet = new Set();
...
<div>
    <span class="text-xl">Selected Repeat for <p class="font-bold inline">{data}</p>.</span>
...
<!--                    add</IconButton>-->
                    <div class="mb-2 inline-block pr-4">
                        <Button
                                on:click={() => readFileData(data, cartRepeats[index].name)}
                                variant="unelevated"
                                class="button-shaped-round"
                        >
                          <Icon class="material-icons">add</Icon>
                          <Label>Add TE Subfamily</Label>
                        </Button>
                    </div>
                    <span class="inline-block">
                            <p class="font-bold">{cartRepeats[index].name}</p>
                            <span class="text-xs">Family: {cartRepeats[index].family}, Class: {cartRepeats[index].class}</span>
                    </span>
                </span>
            </div>
        </VirtualList>
