<script>
  ...
  async function handleSessionDownload(d) {
      ...
      let sessioInput = $Cart.genomelist.filter(x => x.key == d.chr);
      sessioInput[0].values = sessioInput[0].values.filter(x => (x.start == d.start));
      const dataInfo = $Cart.data.filter(x => x.id == sessioInput[0].values[0].data);
      sessioInput[0].values[0].data = dataInfo[0];
      console.log(sessioInput, $Cart.genomelist);
  ...
  function createChromosomeBody(data) {
      ...
      //             div.html(`<p><span class="font-bold">Position:</span> ${d.start}<br/>
